...
$("input[type='radio']").click(function(){
        ...
        $(this).parent().parent().next().find("input[type='text']").bind("blur",function(){
			 ...
			 if($(this).val()==""){
					...
					$(this).parent().next().children().eq(0).html("不能为空");
					$("#next_h").val("0");
					return;
				 }
	   ...
	   if($(this).val() == ""){
		      ...
		      $(this).parent().next().removeClass("display_none");
				  ...
				  if($(this).val().length < 6){
					     $(this).parent().next().find("span").text("至少6位");
		                 $(this).parent().next().removeClass("display_none");
						 return;
					  }
				  if(!reg.test($(this).val())){
						    $(this).parent().next().find("span").text("需数字或字母");
						    $(this).parent().next().removeClass("display_none");
						}else{
	   ...
	   if($(this).val() == ""){
			  ...
			  return;
			      ...
			      if($(this).val() != $("#reset_pwd").val()){
					   $(this).parent().next().find("span").text("密码不相同");
					   $(this).parent().next().removeClass("display_none");
					   return;
				   }
			   }
