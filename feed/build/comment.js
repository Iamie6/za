				...
				for (var prop in cssObj) {
					...
					if (fn.getStyle(elem, prop) == 'transparent' || /^#|rgb\(/.test(fn.getStyle(elem, prop))) {
						...
						if (/^rgb\(/.test(fn.getStyle(elem, prop))) {
							...
							cssOri.push([parseInt(re[0],10), parseInt(re[1],10), parseInt(re[2],10)]);
						}
					} else if (prop == 'opacity') {
						cssOri.push(100 * fn.getStyle(elem, prop));
					} else {
						cssOri.push(parseInt(fn.getStyle(elem, prop),10));
					}
				}
				return cssOri;
			},
			getEndCss: function(cssobj) {
				...
				for (var prop in cssobj) {
					...
					if (prop == 'opacity') {
						...
						cssEnd.push(parseInt(cssobj[prop],10));
					}
				}
				return cssEnd;
			}
		...
		Login.include({
			...
			_setLogin: function() {
				...
				lSTK.Ready(function() {
					...
					// if(!window.SINA_USER_PANEL || !SINA_USER_PANEL.hasBeenInitialed()){
						LoginLayer.init();
						document.title = ('comment login');
					// }
					// \u4E0D\u662F\u767B\u5F55\u72B6\u6001\uFF0C\u5E76\u4E14\u4E0D\u662F\u9884\u767B\u5F55\u72B6\u6001
					if(!self.isLogin()){
						addBodyClz('logout');
			 ...
			 * @private
			 */
			lInSuccess: function(fn) {
				var self = this;
				var opt = self.get('opt');
				_uid = LoginLayer.getSinaCookie().uid || '';
...
// var winSize = $.app.getWinSize(); var scrollTop = document.body.scrollTop || document.documentElement.scrollTop var pos = { 'l': (winSize.width - 477) / 2, 't': scrollTop + (winSize.height - 264) / 2 }; // 477 264 UserPanel.setOutLoginLayerPosition(pos);
			...
			isUidChanged:function(){
				var uid = LoginLayer.getSinaCookie().uid;
				return (_uid !== uid);
			}
